...
> `questionType` 변경 가능 (`MULTIPLE_CHOICE` ↔ `SHORT_ANSWER`).  
> `SHORT_ANSWER`로 바꾸면 기존 객관식 보기는 자동 삭제됩니다.  
> `MULTIPLE_CHOICE`로 바꾸거나 유지할 때는 `options`에 보기 **1개 이상** 필요합니다.  
> 객관식 유지 중 `options`를 넣으면 **전체 교체**, **생략(null)** 시 기존 보기 유지.
...
|----------|------|------|------|
| `questionText` | String | ✅ | 문제 내용 |
| `questionType` | String | ❌ | `MULTIPLE_CHOICE` 또는 `SHORT_ANSWER`. 별칭: `type`, `question_type`. 생략 시 `options`로 추론 |
| `options` | Array | ❌ | 객관식 보기 목록. `[]`이면 단답형으로 간주 (`MULTIPLE_CHOICE`일 때는 1개 이상) |
| `correctAnswer` | String | ✅ | 정답 텍스트 |
| `explanation` | String | ❌ | 해설 |
| `score` | Integer | ✅ | 배점 (0 이상) |
| `anchorId` | Long | ❌ | 참조 교안 ID |
