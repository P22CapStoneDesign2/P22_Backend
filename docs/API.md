...
| Method | URL | 인증 | 설명 |
|--------|-----|------|------|
| `GET` | `/api/admin/lessons` | ✅ | 전체 교안 목록 조회 (페이지네이션) |
| `GET` | `/api/admin/users/pending` | ✅ | 승인 대기 PROF 목록 (페이지네이션) |
| `POST` | `/api/admin/users/{id}/approve` | ✅ | PROF 계정 승인 (ACTIVE로 변경) |
| `POST` | `/api/admin/users/{id}/reject` | ✅ | PROF 계정 거절 (REJECTED로 변경) |
| `PATCH` | `/api/admin/users/{id}/status` | ✅ | PROF 계정 상태 임의 변경 (REJECTED → ACTIVE 등) |
